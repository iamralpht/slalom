...
function makeScrollingExample(parentElement, bunching) {
    ...
    for (var i = 0; i < N; i++) {
        var p = new Slalom.Box('List Item ' + (i+1));
    ...
    context.addMotionConstraint(
        new Slalom.MotionConstraint(firstBox.y, '<=', 0, { physicsModel: Slalom.MotionConstraint.criticallyDamped }));

    // lastBox.bottom >= parentHeight with a critically damped spring.
    context.addMotionConstraint(
        new Slalom.MotionConstraint(lastBox.bottom, '>=', parentHeight, { physicsModel: Slalom.MotionConstraint.criticallyDamped }));
...
makeScrollingExample(document.getElementById('bunching-scrolling-example'), true);
