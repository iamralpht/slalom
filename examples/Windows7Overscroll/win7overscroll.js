...
function makeWindows7Overscroll(parentElement) {
    ...
    var win = new Slalom.Box(parentElement.querySelector('.window'));
    var clip = new Slalom.Box(parentElement.querySelector('.clip'));
    var content = new Slalom.Box(parentElement.querySelector('.image'));
    ...
    context.addMotionConstraint(new Slalom.MotionConstraint(win.y, '==', windowTop, { physicsModel: Slalom.MotionConstraint.criticallyDamped }));

    // Manipulate the content's y.
    context.addManipulator(createManipulator(content.y, parentElement, 'y'));
