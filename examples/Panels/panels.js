...
function makeTwitterPanelsExample(parentElement, constrain) {
    ...
    for (var i = 0; i < 5; i++) {
        var p = new Slalom.Box('Panel ' + (i+1));
        ...
        if (i == 0) {
            ...
            context.addMotionConstraint(new Slalom.MotionConstraint(p.x, '==', 0));
        } else {
            ...
            if (constrain) {
                ...
                context.addMotionConstraint(new Slalom.MotionConstraint(gap, '||', [MIN_GAP, PANEL_WIDTH], { overdragCoefficient: 0, captive: true }));
            }
        }
        panels.push(p);
        context.addBox(p);
    ...
    var manip = createManipulator(lastPanel.x, parentElement, 'x');
